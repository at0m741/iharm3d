...
void diag_flux(struct FluidFlux *F)
{
	...
	{
#if !INTEL_WORKAROUND
#pragma omp parallel for reduction(+ : mdot) reduction(+ : edot) reduction(+ : ldot) reduction(+ : mdot_eh) reduction(+ : edot_eh) reduction(+ : ldot_eh) collapse(2)
#endif
		JSLOOP(0, N2 - 1)
		{
			KSLOOP(0, N3 - 1)
...
#if !INTEL_WORKAROUND
#pragma omp parallel for reduction(+ : rmed) reduction(+ : pp) reduction(+ : e) reduction(max : divbmax) collapse(3)
#endif
		ZLOOP
		{
...
#if !INTEL_WORKAROUND
#pragma omp parallel for reduction(+ : mass_proc) reduction(+ : egas_proc) reduction(+ : Phi_proc) reduction(+ : jet_EM_flux_proc) reduction(+ : lum_eht_proc) collapse(3)
#endif
	ZLOOP
	{
	...
	if (call_code == DIAG_INIT || call_code == DIAG_LOG || call_code == DIAG_FINAL)
	{
		...
		if (mpi_io_proc())
		{
			...
			fprintf(ener_file, "%10.5g %10.5g %10.5g %10.5g %15.8g %15.8g ", t, rmed, pp, e, S->P[UU][N3 / 2][N2 / 2][N1 / 2] * pow(S->P[RHO][N3 / 2][N2 / 2][N1 / 2], -gam), S->P[UU][N3 / 2][N2 / 2][N1 / 2]);
...
double flux_ct_divb(struct GridGeom *G, struct FluidState *S, int i, int j, int k)
{
	...
	{
#endif
		return fabs(0.25 * (S->P[B1][k][j][i] * G->gdet[CENT][j][i] + S->P[B1][k][j - 1][i] * G->gdet[CENT][j - 1][i] + S->P[B1][k - 1][j][i] * G->gdet[CENT][j][i] + S->P[B1][k - 1][j - 1][i] * G->gdet[CENT][j - 1][i] - S->P[B1][k][j][i - 1] * G->gdet[CENT][j][i - 1] - S->P[B1][k][j - 1][i - 1] * G->gdet[CENT][j - 1][i - 1] - S->P[B1][k - 1][j][i - 1] * G->gdet[CENT][j][i - 1] - S->P[B1][k - 1][j - 1][i - 1] * G->gdet[CENT][j - 1][i - 1]) / dx[1] + 0.25 * (S->P[B2][k][j][i] * G->gdet[CENT][j][i] + S->P[B2][k][j][i - 1] * G->gdet[CENT][j][i - 1] + S->P[B2][k - 1][j][i] * G->gdet[CENT][j][i] + S->P[B2][k - 1][j][i - 1] * G->gdet[CENT][j][i - 1] - S->P[B2][k][j - 1][i] * G->gdet[CENT][j - 1][i] - S->P[B2][k][j - 1][i - 1] * G->gdet[CENT][j - 1][i - 1] - S->P[B2][k - 1][j - 1][i] * G->gdet[CENT][j - 1][i] - S->P[B2][k - 1][j - 1][i - 1] * G->gdet[CENT][j - 1][i - 1]) / dx[2] + 0.25 * (S->P[B3][k][j][i] * G->gdet[CENT][j][i] + S->P[B3][k][j - 1][i] * G->gdet[CENT][j - 1][i] + S->P[B3][k][j][i - 1] * G->gdet[CENT][j][i - 1] + S->P[B3][k][j - 1][i - 1] * G->gdet[CENT][j - 1][i - 1] - S->P[B3][k - 1][j][i] * G->gdet[CENT][j][i] - S->P[B3][k - 1][j - 1][i] * G->gdet[CENT][j - 1][i] - S->P[B3][k - 1][j][i - 1] * G->gdet[CENT][j][i - 1] - S->P[B3][k - 1][j - 1][i - 1] * G->gdet[CENT][j - 1][i - 1]) / dx[3]);
	}
	else
	{
		return 0.;
...
void area_map(int i, int j, int k, GridPrim prim)
{
	PLOOP
	{
		...
		fprintf(stderr, "%12.5g %12.5g %12.5g\n", prim[ip][k][j + 1][i - 1], prim[ip][k][j + 1][i], prim[ip][k][j + 1][i + 1]);
		fprintf(stderr, "%12.5g %12.5g %12.5g\n", prim[ip][k][j][i - 1], prim[ip][k][j][i], prim[ip][k][j][i + 1]);
		fprintf(stderr, "%12.5g %12.5g %12.5g\n", prim[ip][k][j - 1][i - 1], prim[ip][k][j - 1][i], prim[ip][k][j - 1][i + 1]);
	}
}
...
void area_map_pflag(int i, int j, int k)
{
	...
	fprintf(stderr, "%d %d %d\n", pflag[k][j + 1][i - 1], pflag[k][j + 1][i], pflag[k][j + 1][i + 1]);
	fprintf(stderr, "%d %d %d\n", pflag[k][j][i - 1], pflag[k][j][i], pflag[k][j][i + 1]);
	fprintf(stderr, "%d %d %d\n", pflag[k][j - 1][i - 1], pflag[k][j - 1][i], pflag[k][j - 1][i + 1]);
}

// TODO this function is useful but slow: it doesn't parllelize under
// intel 18.0.2 Check the whole fluid state for NaN values
