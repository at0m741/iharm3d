...
void mpi_initialization(int argc, char *argv[])
{
	...
	int periodic[3] = {X3L_BOUND == PERIODIC && X3R_BOUND == PERIODIC, X2L_BOUND == PERIODIC && X2R_BOUND == PERIODIC, X1L_BOUND == PERIODIC && X1R_BOUND == PERIODIC};
	...
	if (comm_size != numprocs)
	{
		if (rank == 0)
		{
			...
			fprintf(stderr, "Please reset NiCPU in build_archive/parameters.h "
			                "and recompile, or run iharm3D with a communicator "
	...
	for (int k = -1; k < 2; k++)
	{
		...
		for (int j = -1; j < 2; j++)
		{
			...
			{
				n[2] = coord[2] + i;
				if (((n[0] < 0 || n[0] >= N3CPU) && !periodic[0]) || ((n[1] < 0 || n[1] >= N2CPU) && !periodic[1]) || ((n[2] < 0 || n[2] >= N1CPU) && !periodic[2]))
				{
	...
	if (DEBUG)
	{
		printf("Process %d has X,Y,Z space [%d-%d, %d-%d, %d-%d]\n", rank, global_start[0], global_stop[0], global_start[1], global_stop[1], global_start[2], global_stop[2]);
	}
	...
	MPI_Type_create_subarray(4, sizes, subsizes2, starts, MPI_ORDER_C, scalar_type, &face_type[1]);
	...
	MPI_Type_create_subarray(3, sizes_pflag, subsizes2_pflag, starts_pflag, MPI_ORDER_C, flag_type, &pflag_face_type[1]);
	...
	MPI_Type_create_subarray(4, sizes, subsizes3, starts, MPI_ORDER_C, scalar_type, &face_type[2]);
	MPI_Type_commit(&face_type[2]);

	int subsizes3_pflag[3] = {N3, N2, NG};
	MPI_Type_create_subarray(3, sizes_pflag, subsizes3_pflag, starts_pflag, MPI_ORDER_C, flag_type, &pflag_face_type[2]);
...
int sync_mpi_bound_X1(struct FluidState *S)
{
	...
	MPI_Sendrecv(&(S->P[0][NG][NG][N1]), 1, face_type[2], neighbors[1][1][2], 0, &(S->P[0][NG][NG][0]), 1, face_type[2], neighbors[1][1][0], 0, comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&(pflag[NG][NG][N1]), 1, pflag_face_type[2], neighbors[1][1][2], 6, &(pflag[NG][NG][0]), 1, pflag_face_type[2], neighbors[1][1][0], 6, comm, MPI_STATUS_IGNORE);
	// And back
	MPI_Sendrecv(&(S->P[0][NG][NG][NG]), 1, face_type[2], neighbors[1][1][0], 1, &(S->P[0][NG][NG][N1 + NG]), 1, face_type[2], neighbors[1][1][2], 1, comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&(pflag[NG][NG][NG]), 1, pflag_face_type[2], neighbors[1][1][0], 7, &(pflag[NG][NG][N1 + NG]), 1, pflag_face_type[2], neighbors[1][1][2], 7, comm, MPI_STATUS_IGNORE);
#endif
...
{
#if N2 > 1
	MPI_Sendrecv(&(S->P[0][NG][N2][0]), 1, face_type[1], neighbors[1][2][1], 2, &(S->P[0][NG][0][0]), 1, face_type[1], neighbors[1][0][1], 2, comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&(pflag[NG][N2][0]), 1, pflag_face_type[1], neighbors[1][2][1], 8, &(pflag[NG][0][0]), 1, pflag_face_type[1], neighbors[1][0][1], 8, comm, MPI_STATUS_IGNORE);

	MPI_Sendrecv(&(S->P[0][NG][NG][0]), 1, face_type[1], neighbors[1][0][1], 3, &(S->P[0][NG][N2 + NG][0]), 1, face_type[1], neighbors[1][2][1], 3, comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&(pflag[NG][NG][0]), 1, pflag_face_type[1], neighbors[1][0][1], 9, &(pflag[NG][N2 + NG][0]), 1, pflag_face_type[1], neighbors[1][2][1], 9, comm, MPI_STATUS_IGNORE);
#endif
...
{
#if N3 > 1
	MPI_Sendrecv(&(S->P[0][N3][0][0]), 1, face_type[0], neighbors[2][1][1], 4, &(S->P[0][0][0][0]), 1, face_type[0], neighbors[0][1][1], 4, comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&(pflag[N3][0][0]), 1, pflag_face_type[0], neighbors[2][1][1], 10, &(pflag[0][0][0]), 1, pflag_face_type[0], neighbors[0][1][1], 10, comm, MPI_STATUS_IGNORE);

	MPI_Sendrecv(&(S->P[0][NG][0][0]), 1, face_type[0], neighbors[0][1][1], 5, &(S->P[0][N3 + NG][0][0]), 1, face_type[0], neighbors[2][1][1], 5, comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&(pflag[NG][0][0]), 1, pflag_face_type[0], neighbors[0][1][1], 11, &(pflag[N3 + NG][0][0]), 1, pflag_face_type[0], neighbors[2][1][1], 11, comm, MPI_STATUS_IGNORE);
#endif
