...
#if ELECTRONS
#if ALLMODELS
	const char varNames[NVAR][HDF_STR_LEN] = {
	    "RHO", "UU",   "U1",   "U2",   "U3",   "B1",  "B2",
	    "B3",  "KTOT", "KEL0", "KEL1", "KEL2", "KEL3"};
#else
	const char varNames[NVAR][HDF_STR_LEN] = {
	    "RHO", "UU", "U1", "U2", "U3", "B1", "B2", "B3", "KTOT", "KEL0"};
#endif
#else
	const char varNames[NVAR][HDF_STR_LEN] = {"RHO", "UU", "U1", "U2", "U3",
	                                          "B1",  "B2", "B3"}; // Reserve some extra
#endif
...
void dump_grid(struct GridGeom *G)
{
	...
	for (int d = 0; d < NGRIDVARS; d++)
		x[d] = calloc(1, sizeof(GridDouble));
	const char *coordNames[] = {"X",   "Y",  "Z",  "r", "th",
	                            "phi", "X1", "X2", "X3"};
