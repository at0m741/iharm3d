...
inline void mp5(double x1, double x2, double x3, double x4, double x5, double *lout, double *rout)
{
		...
		PLOOP
		{
			KSLOOP(-1, N3)
			{
				JSLOOP(-1, N2)
				{
					ISLOOP(-1, N1)
					{
						RECON_ALGO(S->P[ip][k][j][i - 2], S->P[ip][k][j][i - 1], S->P[ip][k][j][i], S->P[ip][k][j][i + 1], S->P[ip][k][j][i + 2], &(Pl[ip][k][j][i]), &(Pr[ip][k][j][i]));
					}
				}
			}
		}
		...
		PLOOP
		{
			KSLOOP(-1, N3)
			{
				JSLOOP(-1, N2)
				{
					ISLOOP(-1, N1)
					{
						RECON_ALGO(S->P[ip][k][j - 2][i], S->P[ip][k][j - 1][i], S->P[ip][k][j][i], S->P[ip][k][j + 1][i], S->P[ip][k][j + 2][i], &(Pl[ip][k][j][i]), &(Pr[ip][k][j][i]));
					}
				}
			}
		}
		...
		PLOOP
		{
			KSLOOP(-1, N3)
			{
				JSLOOP(-1, N2)
				{
					ISLOOP(-1, N1)
					{
						RECON_ALGO(S->P[ip][k - 2][j][i], S->P[ip][k - 1][j][i], S->P[ip][k][j][i], S->P[ip][k + 1][j][i], S->P[ip][k + 2][j][i], &(Pl[ip][k][j][i]), &(Pr[ip][k][j][i]));
					}
				}
			}
		}
