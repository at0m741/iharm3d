...
void current_calc(struct GridGeom *G, struct FluidState *S, struct FluidState *Ssave, double dtsave)
{
	...
	ZLOOP
	{
		...
		DLOOP1
		{
			...
			S->jcon[mu][k][j][i] = (1. / (sqrt(4. * M_PI) * 
									G->gdet[CENT][j][i])) * ((gF0p[mu] - gF0m[mu]) / 
									dtsave + (gF1p[mu] - gF1m[mu]) / (2. * dx[1]) + 
									(gF2p[mu] - gF2m[mu]) / (2. * dx[2]) + (gF3p[mu] - 
									gF3m[mu]) / (2. * dx[3]));
		}
	}
...
inline double gFcon_calc(struct GridGeom *G, struct FluidState *S, int mu, int nu, int i, int j, int k)
{
	...
	for (int kap = 0; kap < NDIM; kap++)
		for (int lam = 0; lam < NDIM; lam++)
			Fcon += (-1. / G->gdet[CENT][j][i]) * 
					antisym(mu, nu, kap, lam) * 
					S->ucov[kap][k][j][i] * 
					S->bcov[lam][k][j][i];
