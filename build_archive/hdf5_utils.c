...
hdf5_blob hdf5_get_blob(const char *name)
{
	...
	H5Pclose(plist_id);
	if (file_image_id < 0)
		FAIL(file_image_id, "hdf5_get_blob", name);
...
int hdf5_write_blob(hdf5_blob blob, const char *name)
{
	...
	if (status < 0)
		FAIL(status, "hdf5_write_blob", name);
	return 0;
}
...
int hdf5_close_blob(hdf5_blob blob)
{
	...
	if (status < 0)
		FAIL(status, "hdf5_close_blob", "none");
	return 0;
}
...
int hdf5_write_array(const void *data, const char *name, size_t rank, hsize_t *fdims, hsize_t *fstart, hsize_t *fcount, hsize_t *mdims, hsize_t *mstart, hsize_t hdf5_type)
{
	...
	hid_t dset_id = H5Dcreate(file_id, path, hdf5_type, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
	H5Pclose(plist_id);
...
int hdf5_write_single_val(const void *val, const char *name, hsize_t hdf5_type)
{
	...
	hid_t plist_id = H5Pcreate(H5P_DATASET_CREATE);
	hid_t dset_id = H5Dcreate(file_id, path, hdf5_type, scalarspace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
	H5Pclose(plist_id);
