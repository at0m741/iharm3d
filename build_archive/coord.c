...
{
	*y = 2 * X[2] - 1.;
	*thJ = poly_norm * (*y) * (1. + pow((*y) / 
		   poly_xt, poly_alpha) / 
		   (poly_alpha + 1.)) + 0.5 * M_PI;
}
...
inline void set_dxdX(double X[NDIM], double dxdX[NDIM][NDIM])
{
	memset(dxdX, 0, NDIM * NDIM * sizeof(double));
		...
		dxdX[0][0] = 1.;
		dxdX[1][1] = exp(X[1]);
		dxdX[2][1] = -exp(mks_smooth * (startx[1] - X[1])) * 
						mks_smooth * (M_PI / 2. - M_PI * X[2] + 
						poly_norm * (2. * X[2] - 1.) * (1 + (pow((-1. + 2 * X[2]) / 
						poly_xt, poly_alpha)) / (1 + poly_alpha)) - 1. / 2. * 
						(1. - hslope) * sin(2. * M_PI * X[2]));

		dxdX[2][2] = M_PI + (1. - hslope) * M_PI * cos(2. * M_PI * X[2]) + 
						exp(mks_smooth * (startx[1] - X[1])) * (-M_PI + 2. * 
						poly_norm * (1. + pow((2. * X[2] - 1.) / poly_xt, poly_alpha) / 
						(poly_alpha + 1.)) + (2. * poly_alpha * poly_norm * (2. * X[2] - 1.) * 
						pow((2. * X[2] - 1.) / poly_xt, poly_alpha - 1.)) / ((1. + poly_alpha) *
						poly_xt) - (1. - hslope) * M_PI * cos(2. * M_PI * X[2]));
...
void		set_points()
{
	...
	// then we want xeh = xin + 5.5 * (xout - xin) / N1TOT, or solving/replacing:
		...
		startx[1] = log(Rin);
		if (startx[1] < 0.0)
			ERROR("Not enough radial zones! Increase N1!");
...
void		set_grid(struct GridGeom *G)
{
	...
	JSLOOP(-NG, N2 - 1 + NG)
		ISLOOP(-NG, N1 - 1 + NG)
		{
			...
			{
				if (pow(G->gcon[CENT][0][mu][j][i], 2.) - G->gcon[CENT][mu][mu][j][i] * \
					G->gcon[CENT][0][0][j][i] >= 0.)
				{
