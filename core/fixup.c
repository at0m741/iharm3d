...
#pragma omp parallel for collapse(3) reduction(+ : n_geom_rho) reduction(+ : n_geom_u) reduction(+ : n_b_rho) reduction(+ : n_b_u) reduction(+ : n_temp) reduction(+ : n_gamma) reduction(+ : n_ktot)
	ZLOOP
	{
		int flag = fflag[k][j][i];
		if (flag & HIT_FLOOR_GEOM_RHO)
			n_geom_rho++;
		if (flag & HIT_FLOOR_GEOM_U)
			n_geom_u++;
		if (flag & HIT_FLOOR_B_RHO)
			n_b_rho++;
		if (flag & HIT_FLOOR_B_U)
			n_b_u++;
		if (flag & HIT_FLOOR_TEMP)
			n_temp++;
		if (flag & HIT_FLOOR_GAMMA)
			n_gamma++;
		if (flag & HIT_FLOOR_KTOT)
			n_ktot++;
	}
	...
	LOG("FLOORS:");
	if (n_geom_rho > 0)
		LOGN("Hit %d GEOM_RHO", n_geom_rho);
	if (n_geom_u > 0)
		LOGN("Hit %d GEOM_U", n_geom_u);
	if (n_b_rho > 0)
		LOGN("Hit %d B_RHO", n_b_rho);
	if (n_b_u > 0)
		LOGN("Hit %d B_U", n_b_u);
	if (n_temp > 0)
		LOGN("Hit %d TEMPERATURE", n_temp);
	if (n_gamma > 0)
		LOGN("Hit %d GAMMA", n_gamma);
	if (n_ktot > 0)
		LOGN("Hit %d KTOT", n_ktot);
...
inline void fixup_floor(struct GridGeom *G, struct FluidState *S, int i, int j, int k)
{
	...
	if (rhoflr_geom > S->P[RHO][k][j][i])
		fflag[k][j][i] |= HIT_FLOOR_GEOM_RHO;
	if (uflr_geom > S->P[UU][k][j][i])
		fflag[k][j][i] |= HIT_FLOOR_GEOM_U;
	...
	if (rhoflr_b > S->P[RHO][k][j][i])
		fflag[k][j][i] |= HIT_FLOOR_B_RHO;
	if (uflr_b > S->P[UU][k][j][i])
		fflag[k][j][i] |= HIT_FLOOR_B_U;
	...
	if (rhoflr_temp > S->P[RHO][k][j][i])
		fflag[k][j][i] |= HIT_FLOOR_TEMP; // Misnomer for consistency
...
#pragma omp parallel for simd collapse(2) reduction(+ : nbad_utop)
	...
	if (nleft_utop > 0)
		fprintf(stderr, "Cells STILL BAD after fixup_utoprim: %d\n", nleft_utop);
#endif
